-- Section 24-CREATE FIRST FUNCTION :-

--1.write a function called fix_homepage that updates all suppliers with in homepage field to null.
...
--2.create a function called set_employee_default_photo to update any missing phptopath to a default 'http://accweb/emmployees/default.bmp' and run.
...
--3.write a function that returns a single value :-
...
--find the maximum price of any product.
 ...
 --write a function biggest_order that returns order in terms of total money spent.
...
--find the largest amount given a specified customer.
...
--Find the most ordered product of a particular customer by number of items ordered. call funtcion most_ordered_product.
...
--- IN,OUT,INOUT(BOTH INPUT AND OUTPUT) AND VARIADIC(covered with arrays).
   
   CREATE FUNTCION FUNTCION_NAME (IN X INT , IN Y INT,OUT SUM INT)

---create a function to both add and multiply two number.
...
--create function that take a single number and return the square and cube of the number using OUT parameters. call it square_n_cube.
...
--redo new_price funtcion with a default of 5 % price increase.
...
--redo square_n_cube create a function that takes a single number and returns the square and cube 
...
--Function that return more then one rows

--return all the products that have total sales greater then some input values.
...
--create a funtcion that all the suppliers that have the products that need to be ordered (unitson hand 
...
--PROCEDURE
 ...
 --CREATE A SIMPLE add_em() add two integer.
 ...
 --CREATE A PROCEDURE change_suppliers_price that takes the supplierid and amount and inceases all the units prices in products tables for that suppliers.
